    ...
    <UCard>
      ...
      <div class="space-y-4">
        ...
        <div class="flex flex-wrap gap-3 items-center">
          <USelect v-model="selectedRating" :items="ratingOptions" :placeholder="t('filter.rating')" size="sm" class="w-40" />
          <USelect v-model="selectedCategory" :items="categoryOptions" :placeholder="t('filter.category')" size="sm" class="w-44" />
          <USelect v-model="limit" :items="limitOptions" size="sm" class="w-40" />
          <USelect v-model="language" :items="languageOptions" :placeholder="t('filter.language')" size="sm" class="w-40" />
        </div>

        <div class="flex flex-wrap gap-3 items-center">
          <USelect v-model="selectedStatus" :items="statusOptions" :placeholder="t('filter.status')" size="sm" class="w-44" />
          <USelect v-model="selectedInternal" :items="internalOptions" size="sm" class="w-44" />
        ...
        <div class="space-y-3">
          <div class="flex items-start justify-between gap-4">
            <div class="flex-1">
              ...
              <p class="text-xs text-neutral-400 mt-1">
                {{ claim.shortId }}
                <span v-if="claim.language"> • {{ t('search.languageLabel') }}: {{ claim.language }}</span>
                <span> • {{ t(`status.${claim.status}`, claim.status) }}</span>
                <span v-if="claim.internal" class="text-amber-500"> • {{ t('filter.internalOnly') }}</span>
              </p>
            </div>
          ...
          <div class="flex items-center justify-between">
            <div class="flex flex-wrap gap-1">
              <UBadge v-for="cat in claim.categories" :key="cat" color="neutral" variant="soft" size="xs">
                {{ cat }}
              </UBadge>
            </div>
            <div class="flex items-center gap-2">
              <span class="text-xs text-neutral-400">{{ t('search.score') }}: {{ claim.bestScore.toFixed(4) }}</span>
              <UButton v-if="claim.publishingUrl" :to="claim.publishingUrl" target="_blank" variant="ghost" size="xs" icon="i-heroicons-arrow-top-right-on-square">
                {{ t('common.source') }}
              </UButton>
              <UButton :to="`/claims/${claim.shortId}`" variant="soft" size="xs" icon="i-heroicons-eye">
                {{ t('common.details') }}
              </UButton>
            </div>
          </div>
